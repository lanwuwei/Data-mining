...
def SilhouetteCoefficient(dataInstance,dataInstanceLabel):
    ...
    for i,item in enumerate(data):
        if predictedlabel[i]==dataInstanceLabel:
            ...
            b[predictedlabel[i]]+=euclideanDistance(dataInstance,item)
            count_b[predictedlabel[i]]+=1
    for i in range(k):
        if count_b[i]:
            b[i]=b[i]/count_b[i]
    b.remove(0)
...
def dataAnalysis():
    ...
    Silhouette_overall=sum(Silhouette)/len(data)
    for i in range(k):
        if countCentroids[i]:
    ...
    print SSB_predicted
    print 'SSE predicted'
    print SSE_predict, sum(SSE_predict)
    print 'Cluster Silhouette and Overall Silhouette'
    print Silhouette,Silhouette_overall
    print 'M'
    print M
    #sys.exit()
    c1=[]
...
if __name__=='__main__':
    ...
    #inputFile='wine_cleaned.csv'
    title=[]
    data,actuallabel=readData(inputFile)
    k=2
    centroids,predictedlabel=kMeans(k)
    #predictedlabel=[int(i)-1 for i in actuallabel]
